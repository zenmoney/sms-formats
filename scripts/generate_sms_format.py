...
def _validation_error_to_regex_generation_text(error: ValidationError) -> str:
    ...
    if error.kind == "group_count_mismatch":
        return (
            "group_count_mismatch: captured group count does not match expected entities order/count. "
    ...
    if len(groups) != expected_count:
        return [
            ValidationError(
                ...
                file_path="",
                message=f"Captured groups count mismatch: expected {expected_count}, got {len(groups)}.",
                example_text=sms_text,
            )
        ]
...
async def generate_failed_transaction_regex(
    ...
    failed_transaction_generation_system_prompt_en = dedent(
        """
...
async def generate_otp_regex(
    ...
    otp_generation_system_prompt_en = dedent(
        """
        You are an experienced engineer with strong expertise in regular expressions
        for bank SMS processing.
...
async def validate_regex_flexibility_with_llm(
    ...
    Returns:
        Optional[ValidationError]: ValidationError with all detected problems or None if no problems are found.
    """
    ...
    regex_flexibility_validation_system_prompt_en = dedent(
        r"""
